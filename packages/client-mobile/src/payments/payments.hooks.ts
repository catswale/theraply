...
export const usePayments = () => {
  ...
  async function register(cardToken: string, pkgName: PackageName) {
    const { stripeCustomerID, packageItems } = await callAPI('post', '/payment/register', { cardToken, pkgName });
    updateClient({ stripeCustomerID, packageItems });
  }

  async function addCard(stripeCustomerID: string, cardToken: string) {
    const res = await callAPI('post', '/payment/card', { token: cardToken, stripeCustomerID });
    console.log(res);
  }

  async function charge(stripeCustomerID: string, pkgName: PackageName) {
    const res = await callAPI('post', '/payment/charge', { pkgName, stripeCustomerID });
    console.log(res);
  }
